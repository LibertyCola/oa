...
public partial class system_setSysName : Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        {
            //添加系统用户
            bc.ExecSQL("INSERT INTO sysUser (userName, userPwd, loginTime, system) values('" + dlEmployee.Text + "','" +
                       TextBox1.Text + "','','0')");
            ListBox1.DataBind();
        }
    ...
    {
        //删除系统用户
        bc.ExecSQL("DELETE FROM sysUser WHERE(userName = '" + ListBox1.SelectedItem.Text + "')");
        string strss = Convert.ToString(Session["loginName"]);

        if (ListBox1.SelectedItem.Text.Equals(strss))
        {
            //删除当前用户时,退出系统回到首页
