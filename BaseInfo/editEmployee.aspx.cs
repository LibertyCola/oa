...
public partial class BaseInfo_editEmployee : Page
{
    ...
    {
        //获取要清除相片文件的名称（服务器）
        DataSet ds = bc.GetDataSet("select * from employee where id=" + GridView1.DataKeys[e.RowIndex].Value + "",
                                   "employee");
        DataRow[] row = ds.Tables[0].Select();

        //清除数据
        if (bc.ExecSQL("delete from employee where id=" + GridView1.DataKeys[e.RowIndex].Value + ""))
        {
            GridView1.DataSource = bc.GetDataSet("select * from employee", "employee");
            GridView1.DataKeyNames = new string[] { "id" };
            GridView1.DataBind();
